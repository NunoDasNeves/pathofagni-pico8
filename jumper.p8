...
function num_tiles_until_wall(x,y,dir,vert)
...
function update_shooter(t)
	...
	if t.shooting then
		...
		t.s = t.i + t.s_sh.s
		if play_anim(t, 10, t.s_sh.f) then
			t.shooting = false
			t.fcnt = 0
			t.fr = 0
		elseif t.fr == 2 then
			t.shot = true
			if hit_p(t.x + 4, t.y + 3, 256, 8) then
	...
	-- else we walking
		...
		if (t.shcount <= 0) then
			local left = num_tiles_until_wall(t.x + 1, t.y + 1, -1)
			local right = num_tiles_until_wall(t.x + 1, t.y + 1, 1)
			local width = (left + right + 1) * 8
			dbgstr = tostr(left)..' '..tostr(right)..'\n'..dbgstr
			if hit_p(t.x - width/2, t.y, width, 8) then
				t.shcount = 30
				t.shooting = true
				t.fcnt = 0
				t.fr = 0
...

